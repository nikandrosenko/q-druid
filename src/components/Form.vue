<template>
      <q-card style="min-width: 350px">
        <q-card-section>
          <div v-if="props.updateDeleteType.bool" class="text-h6">Создать модуль</div>
        ...
        <form @submit.prevent="manipulationForm">
          ...
          />
            ...
            <div class="q-pt-md q-px-md" style="max-width: 500px">
              <q-input readonly filled v-model="dateAndTimeStart" label="Начало" :rules="[required]">
            ...
            <div class="q-pt-md q-px-md" style="max-width: 500px">
              <q-input readonly filled v-model="dateAndTimeEnd" label="Конец" :rules="[required]">
                <template v-slot:prepend>
                  <q-icon name="event" class="cursor-pointer">
                    <q-popup-proxy cover transition-show="scale" transition-hide="scale">
          ...
          <q-card-actions align="right" class="text-primary">
            ...
            <q-btn flat label="Создать" v-close-popup type="submit" v-if="props.updateDeleteType.bool"/>
...
import { getGroupSubjects } from "src/graphql/queries.js";
...
const dateAndTimeEnd = computed(() => `${date.value.dateEnd} ${date.value.timeEnd}`)
const dateAndTimeStart = computed(() =>`${date.value.dateStart} ${date.value.timeStart}`)
